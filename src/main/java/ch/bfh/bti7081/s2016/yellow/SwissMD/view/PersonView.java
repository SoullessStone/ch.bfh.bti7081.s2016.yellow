...
public class PersonView extends CustomComponent implements View {
	...
	public void enter(ViewChangeEvent event) {
		...
		medis.add(new PrescriptionDTO(new DrugDTO("Alcacyl"), 50,
				new DateRange(new Date(), new Date(
						new Date().getTime() + 1000000))));
		medis.add(new PrescriptionDTO(new DrugDTO("Gift"), 50, new DateRange(
				new Date(), new Date(new Date().getTime() + 1000000))));

		layout.addComponent(new MeetingTile(new MeetingDTO(new PatientDTO(
				"Franz Erl", new Date(-620438400000L)), new DoctorDTO(
				"Doktor Who", new Date(335145600000L)),
				new Date(1457802000000L))));
